...
class Hotel(DBbase):
        ...
        if self.get_cursor.fetchone()[0] == 0:
            for room_type, column in hotel.items():
                self.get_cursor.execute(
                    "INSERT INTO rooms (room_type, total_rooms, available, rate) VALUES (?, ?, ?, ?)",
        ...
        while True:
            ...
            try:
                ...
                if response == 1:
                    ...
                    print("Thank you for visiting the Grand Py Hotel!")
                    break
                else:
    ...
    def find_next_available_date(self, room_type, check_in, nights):
        ...
        while True:
            ...
            is_available, available_count = self.is_room_available(room_type, current_date.strftime("%Y-%m-%d"),
                                                                   check_out)
            if is_available:
    ...
    def find_available_rooms_on_date(self, check_in, nights):
        ...
        for room_type in hotel.keys():
            is_available, available_count = self.is_room_available(room_type, check_in,
                                                                   check_in + datetime.timedelta(days=nights).strftime(
                                                                       "%Y-%m-%d"))
            if is_available:
            ...
            try:
                ...
                if room_type in hotel:
                    ...
                    if is_available:
                        ...
                        self.get_cursor.execute(
                            "INSERT INTO bookings (room_type, check_in, check_out) VALUES (?, ?, ?)",
                        ...
                        if receipt_request == 'y':
                            ...
                            self.print_receipt(name, reservation_id, room_type, check_in,
                                               check_out_date.strftime("%Y-%m-%d"), total_cost)
                        break

                    # if the original requested date isn't available, this finds the next open date for that specific room
                    else:
                        next_available_date, available_count = self.find_next_available_date(room_type, check_in,
                                                                                             nights)
                        print(f"Sorry, the {room_type} room is not available for the selected dates.")
                        print(
                            f"The next available date for a {nights}-night stay in the {room_type} room is: {next_available_date}.")
                        # option to rebook that room type at a later date
                        confirm_booking = input(
                            f"Would you like to book the {room_type} room on {next_available_date}? (Y/N): ").lower()
                        if confirm_booking == 'y':
                            check_out_date = datetime.datetime.strptime(next_available_date,
                                                                        "%Y-%m-%d") + datetime.timedelta(days=nights)
                            total_cost = hotel[room_type]['rate'] * nights
                            ...
                            receipt_request = input("Would you like a receipt? (Y/N): ").lower()
                            if receipt_request == 'y':
                                self.print_receipt(room_type, next_available_date, check_out_date.strftime("%Y-%m-%d"),
                                                   total_cost)
                            break
